...
let TipBot = function (bot, RPC_USER, RPC_PASSWORD, RPC_PORT, OPTIONS) {
    ...
    self.HighBalanceWarningMark = pivx.toDuff(2.0);
    ...
    self.OPTIONS = _.defaults(OPTIONS, {
        ...
        TX_FEE: pivx.toDuff(0.0001), // TX fee, used in withdrawing, in Duffs
        WALLET_PASSW: null,
        ...
        SUN_DEFAULT_THRESHOLD: pivx.toDuff(5) // duff
    });
    ...
    self.CURRENCY_REGEX = new RegExp('\\b(duff?|PIVX|' + self.CURRENCIES.join('|') + ')\\b', 'ig'); // added \b: bugfix for only finding the currencr symbol instead parts of words (aud = audit)
            ...
            if (user.is_admin) {
                ...
                if (message.match(/\bthreshold\b/i)) {
                    ...
                    if (amount !== null) {
                        ...
                        sun.saveThreshold(pivx.toDuff(amount[1]),
                            function (err) {
                                if (err) {
                                    debug(err); return;
                                } else {
