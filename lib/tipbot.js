...
let TipBot = function (bot, RPC_USER, RPC_PASSWORD, RPC_PORT, OPTIONS) {
    ...
    self.CURRENCY_REGEX = new RegExp('\\b(duff?|DASH|' + self.CURRENCIES.join('|') + ')\\b', 'ig'); // added \b: bugfix for only finding the currencr symbol instead parts of words (aud = audit)
...
TipBot.prototype.normalizeValue = function (inputValue, unit, user, cb) {
    ...
    if (inputValue === 'all' && user !== undefined) {
        user.getBalance(user.id, 6, function (err, balance) {
            ...
            cb(value, null, null, null, pivx.toPivx(value) + ' DASH');
        });
    ...
    if (currency !== 'DASH') {
        ...
        self.getPriceRates(function (err, rates) {
            ...
            if (!rate) {
                ...
                let text = value.toFixed(2) + ' ' + currency + ' ' +
                    '(' + pivx.toPivx(newValue) + ' DASH at ' + rate.toFixed(2) + ' ' + currency + ' / DASH)';
        ...
        return cb(value, null, null, null, pivx.toPivx(value) + ' DASH');
    }
};
