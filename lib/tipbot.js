...
let TipBot = function (bot, RPC_USER, RPC_PASSWORD, RPC_PORT, OPTIONS) {
    ...
    self.HighBalanceWarningMark = tx.toDuff(2.0);
    ...
    self.OPTIONS = _.defaults(OPTIONS, {
        ...
        TX_FEE: tx.toDuff(0.0001), // TX fee, used in withdrawing, in Duffs
        WALLET_PASSW: null,
        ...
        SUN_DEFAULT_THRESHOLD: tx.toDuff(5) // Duff
    });

    // create connection via RPC to wallet
    self.wallet = new txd.Client({
        host: 'localhost',
        port: RPC_PORT,
        user: RPC_USER,
        pass: RPC_PASSWORD,
    ...
    self.CURRENCY_REGEX = new RegExp('\\b(duff?|TX|' + self.CURRENCIES.join('|') + ')\\b', 'ig'); // added \b: bugfix for only finding the currencr symbol instead parts of words (aud = audit)

    self.AMOUNT_REGEX = new RegExp('\\s(\\d+\\.\\d{1,8}|\\.\\d{1,8}|\\d+)(?:\\s|$)');
    self.AMOUNT_OR_ALL_REGEX = new RegExp('\\s(\\d+\\.\\d{1,8}|\\.\\d{1,8}|\\d+|all)(?:\\s|$)');

    self.ADDRESS_REGEX = new RegExp('[T|y][a-zA-Z0-9]{25,36}', 'g');
    ...
    fs.exists(filename, function (exists) {
        if (exists) {
         ...
         //   request.get('http://coinmarketcap-nexuist.rhcloud.com/api/dnet/price', function (err, response, body) {
            request.get('http://coinmarketcap-nexuist.rhcloud.com/api/tx/price', function (err, response, body) {
...
// return via callback amount in tx, and if it was needed to convertion rate and originalCurrency
...
TipBot.prototype.normalizeValue = function (inputValue, unit, user, cb) {
    ...
    if (inputValue === 'all' && user !== undefined) {
        user.getBalance(user.id, 6, function (err, balance) {
            ...
            let value = tx.toDuff(balance);
            debug('Log: using ALL balance of ' + user.name + ' = ' + balance);
            cb(value, null, null, null, tx.toTX(value) + ' TX');
        });
        ...
        } else if (unit.match(/TX/i)) {
            currency = 'TX';
            value = tx.toDuff(inputValue);
        } else {
        ...
        self.getPriceRates(function (err, rates) {
            ...
            if (!rate) {
                ...
                let text = value.toFixed(2) + ' ' + currency + ' ' +
                    '(' + tx.toTX(newValue) + ' TX at ' + rate.toFixed(2) + ' ' + currency + ' / TX)';
                // return converted value in tx,  convertion rate, originalCurrency, originalValue, text about the convertion
                return cb(newValue, rate, currency, value, text);
            }
        });
    } else {
        // amount is in Tx, return only value, no convertion rate
        return cb(value, null, null, null, tx.toTX(value) + ' TX');
    }
};
                            ...
                            self.getDirectMessageChannelID(null, oneUser.id, function (err, DMchannelRecievingUser) {
                                if (err === null) {
                                    ...
                                    let recievingUserMessage = {
                                        ...
                                        'text': texts.SunRecieved + tx.toTX(sunraySize) + ' tx'
                                    };
...
TipBot.prototype.onMessage = function (channel, member, message) {
    ...
    self.getDirectMessageChannelID(channel, user.id, function (err, DMchannelID) {
        ...
        if (message.match(/\bdeposit\b/i)) {
            user.tellDepositAddress(function (err, line) {
                if (err) {
        ...
        if (message.match(/\bwithdraw\b/i)) {
            ...
            if (providedCurrency !== null && providedCurrency[0].length !== 0) {
                ...
                //not provided, set tx as default currency
                amount[2] = 'TX';
            }
            debug(amount);
            ...
            // convert amount if currency isn't TX
                ...
                if (fromCurrency === 'tx') {
                    ...
                    self.normalizeValue(amount[1], toCurrency, user, function (value, rate) {
                        if (rate === false) {
                            ...
                            reply.text = amount[1] + ' ' + fromCurrency + ' = ' + newValue.toPrecision(4) + '  ' + toCurrency +
                                ' ( 1.0 Tx = ' + rate.toPrecision(4) + ' ' + toCurrency + ' )';
                        }
                        self.slack.say(reply);
                    });
                } else if (toCurrency === 'tx') {
                    // other -> tx
                    self.normalizeValue(amount[1], fromCurrency, user, function (value, rate) {
                        if (rate === false) {
                            ...
                            reply.text = amount[1] + ' ' + fromCurrency + ' = ' + tx.toTX(value) + '  ' + toCurrency +
                                ' ( 1.0 ' + fromCurrency + ' = ' + (1.0 / rate).toPrecision(4) + ' TX)';
                        }
                        self.slack.say(reply);
                    });
                }
            ...
            sun.getSunBalance(self.sunUser, function (err, sunBalance) {
                if (err) {
                    ...
                    if (sunBalance !== undefined && sunBalance > 2e-8) {
                        reply.text = texts.SunAvailibleAmount + sunBalance + ' tx';
                    } else {
            ...
            if (user.is_admin) {
                ...
                if (message.match(/\bthreshold\b/i)) {
                    ...
                    if (amount !== null) {
                        ...
                        sun.saveThreshold(tx.toDuff(amount[1]),
                            function (err) {
                                if (err) {
                                    debug(err); return;
                                } else {
