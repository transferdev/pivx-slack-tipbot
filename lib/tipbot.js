...
let fs = require('fs');
let pivxd = require('bitcoin');
let texts = require('../text/txt_pivx.js').tipbotTxt;
let sun; // only required if ENABLE_SUN_FEATURE
let quiz; // only required if  ENABLE_QUIZ_FEATURE
let waits = require('waitjs');
let pivx = require('./pivx');

const BLACKLIST_CURRENCIES = ['PIVX'];
...
let TipBot = function (bot, RPC_USER, RPC_PASSWORD, RPC_PORT, OPTIONS) {
    ...
    self.HighBalanceWarningMark = pivx.toPivit(2.0);
    ...
    self.OPTIONS = _.defaults(OPTIONS, {
        ...
        TX_FEE: pivx.toPivit(0.0001), // TX fee, used in withdrawing, in Pivits
        WALLET_PASSW: null,
        ...
        SUN_DEFAULT_THRESHOLD: pivx.toPivit(5) // pivit
    });

    // create connection via RPC to wallet
    self.wallet = new pivxd.Client({
        host: 'localhost',
        port: RPC_PORT,
        user: RPC_USER,
        pass: RPC_PASSWORD,
    ...
    self.CURRENCY_REGEX = new RegExp('\\b(pivit?|PIVX|' + self.CURRENCIES.join('|') + ')\\b', 'ig'); // added \b: bugfix for only finding the currencr symbol instead parts of words (aud = audit)

    self.AMOUNT_REGEX = new RegExp('\\s(\\d+\\.\\d{1,8}|\\.\\d{1,8}|\\d+)(?:\\s|$)');
    self.AMOUNT_OR_ALL_REGEX = new RegExp('\\s(\\d+\\.\\d{1,8}|\\.\\d{1,8}|\\d+|all)(?:\\s|$)');

    self.ADDRESS_REGEX = new RegExp('[D|y][a-zA-Z0-9]{25,36}', 'g');
 ...
 if (self.OPTIONS.ENABLE_SUN_FEATURE) {
  ...
  if (self.OPTIONS.ENABLE_QUIZ_FEATURE) {
    quiz = require('./quiz');
  }
    // Init tipbot
    self.init();
...
TipBot.prototype.tellPrice = function (currency, cb) {
    ...
    self.getPriceRates(function (err, rates) {
        ...
        if (!rate) {
            ...
            cb(null, texts.PriceBase + Number(rate).toPrecision(4) + ' ' + currency);
        }
    });
};
...
TipBot.prototype._getPriceRates = function (filename, cb) {
    fs.exists(filename, function (exists) {
        if (exists) {
         ...
         //   request.get('http://coinmarketcap-nexuist.rhcloud.com/api/dnet/price', function (err, response, body) {
            request.get('http://coinmarketcap-nexuist.rhcloud.com/api/pivx/price', function (err, response, body) {
...
// return via callback amount in pivx, and if it was needed to convertion rate and originalCurrency
...
TipBot.prototype.normalizeValue = function (inputValue, unit, user, cb) {
    ...
    if (inputValue === 'all' && user !== undefined) {
        user.getBalance(user.id, 6, function (err, balance) {
            ...
            let value = pivx.toPivit(balance);
            debug('Log: using ALL balance of ' + user.name + ' = ' + balance);
            cb(value, null, null, null, pivx.toPivx(value) + ' PIVX');
        });

    } else {
        // no 'all', evaluate the unit
        if (unit.match(/pivit?/i)) {
            currency = 'PIVX';
            value = parseInt(inputValue);
        } else if (unit.match(/PIVX/i)) {
            currency = 'PIVX';
            value = pivx.toPivit(inputValue);
        } else {
        ...
        self.getPriceRates(function (err, rates) {
            ...
            if (!rate) {
                ...
                let text = value.toFixed(2) + ' ' + currency + ' ' +
                    '(' + pivx.toPivx(newValue) + ' PIVX at ' + rate.toFixed(2) + ' ' + currency + ' / PIVX)';
                // return converted value in pivx,  convertion rate, originalCurrency, originalValue, text about the convertion
                return cb(newValue, rate, currency, value, text);
            }
        });
    } else {
        // amount is in Pivx, return only value, no convertion rate
        return cb(value, null, null, null, pivx.toPivx(value) + ' PIVX');
    }
};
                            ...
                            self.getDirectMessageChannelID(null, oneUser.id, function (err, DMchannelRecievingUser) {
                                if (err === null) {
                                    ...
                                    let recievingUserMessage = {
                                        ...
                                        'text': texts.SunRecieved + pivx.toPivx(sunraySize) + ' pivx'
                                    };
    ...
    self.getDirectMessageChannelID(channel, user.id, function (err, DMchannelID) {
        ...
        if (message.match(/\bwithdraw\b/i)) {
            ...
            if (providedCurrency !== null && providedCurrency[0].length !== 0) {
                ...
                //not provided, set pivx as default currency
                amount[2] = 'PIVX';
            }
            debug(amount);
            ...
            // convert amount if currency isn't Pivx
        ...
        if (message.match(/\b(send|give|sent|tip)\b/i)) {
            ...
            if (userMatches.length === 0) {
                ...
                // convert if currency isn't Pivx
                ...
                if (fromCurrency === 'pivx') {
                    ...
                    self.normalizeValue(amount[1], toCurrency, user, function (value, rate) {
                        if (rate === false) {
                            ...
                            reply.text = amount[1] + ' ' + fromCurrency + ' = ' + newValue.toPrecision(4) + '  ' + toCurrency +
                                ' ( 1.0 Pivx = ' + rate.toPrecision(4) + ' ' + toCurrency + ' )';
                        }
                        self.slack.say(reply);
                    });
                } else if (toCurrency === 'pivx') {
                    // other -> pivx
                    self.normalizeValue(amount[1], fromCurrency, user, function (value, rate) {
                        if (rate === false) {
                            ...
                            reply.text = amount[1] + ' ' + fromCurrency + ' = ' + pivx.toPivx(value) + '  ' + toCurrency +
                                ' ( 1.0 ' + fromCurrency + ' = ' + (1.0 / rate).toPrecision(4) + ' PIVX)';
                        }
                        self.slack.say(reply);
                    });
                }
            ...
            sun.getSunBalance(self.sunUser, function (err, sunBalance) {
                if (err) {
                    ...
                    if (sunBalance !== undefined && sunBalance > 2e-8) {
                        reply.text = texts.SunAvailibleAmount + sunBalance + ' pivx';
                    } else {
            ...
            if (user.is_admin) {
                ...
                if (message.match(/\bthreshold\b/i)) {
                    ...
                    if (amount !== null) {
                        ...
                        sun.saveThreshold(pivx.toPivit(amount[1]),
                            function (err) {
                                if (err) {
                                    debug(err); return;
                                } else {
        ...
        if (message.match(/\bquiz\b/i)) {
            ...
            if (message.match(/\banswer\b/i)) {
                let answer = message.replace('answer', '').replace('quiz', '').trim();
                //   reply.text = 'A:' + answer + ':A';
                quiz.checkAnswer(answer, user.name,
                    function (err, done, correct) {
                        if (err) {
                            debug(err);
                            reply.text = err;
                            self.slack.say(reply);
                            return; // TODO why isn't this return working?
                        }
                        if (correct) {
                            // TODO refactor in own function?
                            // correct answer
                            reply.text = texts.QuizAnswerCorrect;
                            if (done) {
                                ...
                                quiz.end(function (scores) {
                                    ...
                                    _.forEach(scores, (score, username) => {
                                        let scoreText = username + texts.QuizScored + score + '\n';
                                        debug(scoreText);
                                ...
                                // show next question
                                self.askQuizQuestion(quiz, reply);
                            }
                        } else {
                            // not correct answer
                            reply.text = texts.QuizAnswerWrong + '\n' +
                                texts.QuizAskQuestion1 + self.slack.identity.name + texts.QuizAskQuestion2;
                            self.slack.say(reply);
                        }
                    });
                //  self.slack.say(reply);
            ...
            if (user.is_admin) {
                        ...
                        if (questions.length === 0) {
                            ...
                            questions.forEach(function (QandA) {
                                questionsSentence += '*' + QandA.qaNumber + '*: ' +
                                    QandA.question + '\n' + texts.QuizListReward_1 +
                                    QandA.reward + texts.QuizListReward_2 +
                                    '\n';
                            });
                ...
                if (message.match(/\breview\b/i)) {
                    quiz.getUnreviewedQuestions(function (err, questions) {
                        ...
                        if (questions.length === 0) {
                            ...
                            questions.forEach(function (QandA) {
                                questionsSentence += QandA.qaNumber + ' ```' +
                                    texts.QuizQ + QandA.question + '\n' +
                                    texts.QuizA + QandA.answer + ' ``` ';
                            });
                            // tell how set a reward for a question
                            questionsSentence += '\n\n' + texts.QuizRewardQuestion_1 + self.slack.identity.name + texts.QuizRewardQuestion_2;
                            privateReply.text = questionsSentence;
                            self.slack.say(privateReply);
                        }
                    });
                ...
                // start a quiz(pick questions and ask first one)
                if (message.match(/\bstart\b/i)) {
                    quiz.start(function (err) {
                        if (err) {
                ...
                // to skip a question (if no one finds the answer)
                if (message.match(/\bnext\b/i)) {
                    self.askQuizQuestion(quiz, reply);
                }
...
TipBot.prototype.askQuizQuestion = function (quiz, reply) {
    let self = this;
    quiz.askQuestion(function (err, questionSentence) {
        if (err) {
            debug('ERROR cannot tell a quiz question: ' + err);
            return;
        }
        reply.text = questionSentence + '\n' +
            texts.QuizAskQuestion1 + self.slack.identity.name + texts.QuizAskQuestion2;
        self.slack.say(reply);
    });
};
