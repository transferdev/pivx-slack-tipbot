...
let helpTexts = require('../text/txt_pivx.js').tipbotTxt;
...
function getSunRaySize(sunBalance, cb) {
    getAmountOfEligibleSunUsers(
        (err, amountOfTippers) => {
            ...
            let sunraySize = pivx.toPivit(sunBalance) / amountOfTippers;
            sunraySize -= 1; // 1 pivit to prevent rouding errors so the last sunray is still enough
            debug('SUN: ' + amountOfTippers + ' will recieve ' + pivx.toPivx(sunraySize));
            cb(null, sunraySize);
        });
}
    ...
    getSunRaySize(sunBalance, function (err, sunraySize) {
        ...
        getListOfSunEligibleUsers(function (err, usersList) {
            async.forEachSeries(usersList,
                (oneUser, asyncCB) => {
                    debug('Cast a sunray of ' + pivx.toPivx(sunraySize) + ' pivx on ' + oneUser.name + ' (' + oneUser.id + ')');
        ...
        (err, threshold) => {
            ...
            getSunBalance(sunUser,
                function (err, sunBalance) {
                    if (pivx.toPivit(sunBalance) >= threshold) {
...
// get saved threshold (in Pivits), if not saved us default threshold
