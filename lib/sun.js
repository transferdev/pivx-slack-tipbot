...
let helpTexts = require('../text/txt_tx.js').tipbotTxt;
let tx = require('./tx');
...
function getSunRaySize(sunBalance, cb) {
    getAmountOfEligibleSunUsers(
        (err, amountOfTippers) => {
            ...
            let sunraySize = tx.toDuff(sunBalance) / amountOfTippers;
            sunraySize -= 1; // 1 pivit to prevent rouding errors so the last sunray is still enough
            debug('SUN: ' + amountOfTippers + ' will recieve ' + tx.toTX(sunraySize));
            cb(null, sunraySize);
        });
}
    ...
    getSunRaySize(sunBalance, function (err, sunraySize) {
        ...
        getListOfSunEligibleUsers(function (err, usersList) {
            async.forEachSeries(usersList,
                (oneUser, asyncCB) => {
                    debug('Cast a sunray of ' + tx.toTX(sunraySize) + ' tx on ' + oneUser.name + ' (' + oneUser.id + ')');
        ...
        (err, threshold) => {
            ...
            getSunBalance(sunUser,
                function (err, sunBalance) {
                    if (tx.toDuff(sunBalance) >= threshold) {
