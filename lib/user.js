...
let helpText = require('../text/txt_pivx.js').userTxt;
...
let User = function (tipbot, userId, userName, isAdmin) {
    ...
    self.blockchainUrl = 'http://www.presstab.pw/phpexplorer/DNET/tx.php';
    // self.blockchainUrl = 'https://chainz.cryptoid.info/dash/tx.dws?'; // NEED TO CHANGE TO PRESSTAB.PW
...
function sendAmount(self, toAddress, value, cb) {
    ...
    self.wallet.sendFrom(self.id, toAddress, parseFloat(pivx.toPivx(value)),
        function (err, tx_id) {
            if (err) {
            ...
            let line = helpText.Withdrawal1 + pivx.toPivx(value) + ' ' +
...
User.prototype.getBalanceLine = function (cb) {
    ...
    self.getBalance(self.id, self.REQUIRED_CONFIRMATIONS, function (err, balance) {
        ...
        if (pivx.toPivit(balance) >= self.tipbot.HighBalanceWarningMark) {
            // warn user
...
User.prototype.send = function (sendToUser, value, cb) {
    ...
    self.wallet.getBalance(self.id, self.REQUIRED_CONFIRMATIONS, function (err, balance) {
        ...
        self.wallet.move(self.id, sendToUser.id, parseFloat(pivx.toPivx(value)), function (err, result) {
            if (err) {
                debug('ERROR: moving between account (' + self.id + ') to acount (' + sendToUser.id + ')' + err);
                error = helpText.SendOops1 + pivx.toPivx(value) + ' ' + helpText.BaseCurrency + helpText.SendOops2 + sendToUser.name + '';
                self.locked = false; //  this transaction is done, clear lock to allow new transactions
                cb(error, null);
            ...
            if (result === true) {
                ...
                debug('Sending Tip: Moved ' + pivx.toPivx(value) + ' Pivx from ' + self.handle + ' to ' + sendToUser.name + '(' + sendToUser.id + ')');
                ...
                responses.privateToReciever = helpText.SendPrivateMssRecievingUser1 + sendToUser.handle +
                    helpText.SendPrivateMssRecievingUser2 + pivx.toPivx(value) + ' ' +
