...
let helpText = require('../text/txt_tx.js').userTxt;
let tx = require('./tx');
...
let User = function (tipbot, userId, userName, isAdmin) {
    ...
    self.blockchainUrl = 'https://chainz.cryptoid.info/tx/tx.dws?';
...
function sendAmount(self, toAddress, value, cb) {
    ...
    self.wallet.sendFrom(self.id, toAddress, parseFloat(tx.toTX(value)),
        function (err, tx_id) {
            if (err) {
            ...
            let line = helpText.Withdrawal1 + tx.toTX(value) + ' ' +
...
User.prototype.getBalanceLine = function (cb) {
    ...
    self.getBalance(self.id, self.REQUIRED_CONFIRMATIONS, function (err, balance) {
        ...
        if (tx.toDuff(balance) >= self.tipbot.HighBalanceWarningMark) {
            // warn user
...
User.prototype.tellDepositAddress = function (cb) {
    let self = this;
    ...
    self.wallet.getaddresesbyaccount(self.id, function (err, addresses) {
        if (err) { cb(err, null); }
...
User.prototype.send = function (sendToUser, value, cb) {
    ...
    self.wallet.getBalance(self.id, self.REQUIRED_CONFIRMATIONS, function (err, balance) {
        ...
        self.wallet.move(self.id, sendToUser.id, parseFloat(tx.toTX(value)), function (err, result) {
            if (err) {
                debug('ERROR: moving between account (' + self.id + ') to acount (' + sendToUser.id + ')' + err);
                error = helpText.SendOops1 + tx.toTX(value) + ' ' + helpText.BaseCurrency + helpText.SendOops2 + sendToUser.name + '';
                self.locked = false; //  this transaction is done, clear lock to allow new transactions
                cb(error, null);
            ...
            if (result === true) {
                ...
                debug('Sending Tip: Moved ' + tx.toTX(value) + ' Tx from ' + self.handle + ' to ' + sendToUser.name + '(' + sendToUser.id + ')');
                ...
                responses.privateToReciever = helpText.SendPrivateMssRecievingUser1 + sendToUser.handle +
                    helpText.SendPrivateMssRecievingUser2 + tx.toTX(value) + ' ' +
