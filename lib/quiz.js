...
let texts = require('../text/txt_pivx.js').quizTxt;

let amountQuizQuestions = 2;
let quizQuestions = null;
let scoreboard = {}; 	// username : points
let participants = [];  // only 1 try of each answer
let isRunning = false;
...
function askQuestion(cb) {
    ...
    // clear participants for this question
    participants = [];
...
function checkAnswer(answer, userName, cb) {
    ...
    if (participants.indexOf(userName) !== -1 ) {
        cb(userName + texts.alreadyAnswered,null,null);
        return;
    }
    // store user as taken a try at this question
    participants.push(userName);
    // check provided answer for this question
